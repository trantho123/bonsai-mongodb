...
const CheckoutForm = () => {
    ...
    const getUserData = async () => {
        try {
            ...
            if (!data.data.address || !data.data.city || !data.data.zipCode || !data.data.userState) {
                setOpenAlert(true);
                console.log(1);
              }
            userDetails.firstName = data.data.firstName
            userDetails.lastName = data.data.lastName
            userDetails.userEmail = data.data.email
            userDetails.phoneNumber = data.data.phone
            userDetails.address = data.data.address
            userDetails.zipCode = data.data.zipCode
            userDetails.city = data.data.city
    ...
    const checkOutHandler = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                navigate(`/paymentsuccess?reference=${response.data.orderId}`)
            }
        } catch (error) {
            console.error(error)
